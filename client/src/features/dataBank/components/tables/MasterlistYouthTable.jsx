...
import { YOUTH_MASTERLIST_HEADERS } from '../../utils/constants';

import { useNotification } from '../../hooks/useNotification';



const MasterlistYouthTable = ({ barangay }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await get(`/databank/youth/masterlist/${barangay}`);
      setData(response);
    } catch (err) {
  ...
  return (
    <>
      <ManageTable
        headers={YOUTH_MASTERLIST_HEADERS}
        data={data}
        loading={loading}
        renderRow={renderRow}
...
export default MasterlistYouthTable;