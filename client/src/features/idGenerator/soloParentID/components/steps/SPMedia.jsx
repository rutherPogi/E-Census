...
import MediaUpload from "../others/SPMediaSection/MediaUpload";
import SignatureCapture from "../others/SPMediaSection/SignaturePad";




export default function SPMedia({ handleBack, handleNext }) {
  ...
  useEffect(() => {
    ...
    if (formData.spMedia) {
      // Handle Photo ID
      if (formData.spMedia.photoID) {
        setValues(prev => ({ ...prev, photoID: formData.spMedia.photoID }));
        if (formData.spMedia.photoID instanceof File) {
          setPhotoIDPreview(URL.createObjectURL(formData.spMedia.photoID));
        } else if (formData.spMedia.photoIDPreview) {
          setPhotoIDPreview(formData.spMedia.photoIDPreview);
        }
      }
      
      // Handle Signature
      if (formData.spMedia.signature) {
        setValues(prev => ({ ...prev, signature: formData.spMedia.signature }));
      }
    }
  }, [formData.spMedia]);
      ...
      <div className='responsive-form details'>
        ...
        <FormControl fullWidth margin="normal">
          ...
          <SignatureCapture
            ...
            initialSignature={formData.spMedia?.signature}
          />
        </FormControl>
      </div>
      <FormButtons
