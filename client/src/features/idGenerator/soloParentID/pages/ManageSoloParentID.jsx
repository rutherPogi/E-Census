...
const ManageSoloParentID = () => {
  ...
  const handleSelectApplication = (spApplicationID) => {
    setSelectedApplication(prev => {
      if (prev.includes(spApplicationID)) {
        return prev.filter(id => id !== spApplicationID);
      } else {
        return [...prev, spApplicationID];
      }
    });
  };
  ...
  const handleSelectAllApplication = () => {
    const displayedApplications = filteredData
      ...
      .map(application => application.spApplicationID);
  ...
  const currentPageApplication = filteredData
    .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
    .map(survey => survey.spApplicationID);
  ...
  const deleteMultipleApplications = async () => {
    ...
    try {
      ...
      const applicationsToDelete = applicationData.filter(application => 
        selectedApplication.includes(application.spApplicationID)
      );
  ...
  const renderApplicationRow = (application, index) => (
    <TableRow key={application.spApplicationID || index}>
      <TableCell padding="checkbox">
        <Checkbox
          checked={selectedApplication.includes(application.spApplicationID)}
          onChange={() => handleSelectApplication(application.spApplicationID)}
          color="primary"
        />
      </TableCell>
          ...
          <Tooltip title="View Application Details">
            <Box>
              <ActionButton 
                ...
                label="View"
                color="#0d47a1"
                to={`/main/generate-id/solo-parent/view/${application.spApplicationID}/`}
              />
            </Box>
          </Tooltip>
    ...
    >
      <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center'}}>
          <People/>
          <Typography variant='h5' fontWeight={'bold'}>SOLO PARENT ID APPLICATIONS</Typography>
        </Box>
      </Box>
