...
const ManagePopulation = () => {
  ...
  const fetchSurveyData = async () => {
    setLoading(true);
    try {
      const response = await get('/population/manage-population');
      ...
      setNotification({
        open: true,
        message: err.response?.data?.error || 'Failed to load data. Please try again later.',
        severity: 'error'
      });
      setSurveyData([]);
      setFilteredData([]);
    ...
    <TableRow key={survey.populationID || index}>
      ...
      <TableCell>
        {survey.firstName + ' ' +
         (survey.middleName || '') + ' ' +
         survey.lastName + 
         (survey.suffix && survey.suffix !== 'N/A' ? survey.suffix : ' ')}
      </TableCell>
      <TableCell>
        {survey.birthdate ? dayjs(survey.birthdate).format('MM/DD/YYYY') : 'N/A'}
      </TableCell>
      <TableCell>{survey.age}</TableCell>
      <TableCell>{survey.civilStatus}</TableCell>
